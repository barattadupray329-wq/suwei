...
---
## 🌐 Web 版本启动方式
本项目现在支持一个轻量级 Web 入口，适合小数据量、先快速上线。
...
```powershell
$env:SUWEI_WEB=1
$env:SUWEI_WEB_HOST="0.0.0.0"
$env:SUWEI_WEB_PORT="8000"
python main.py
```

### 生产部署建议
- 服务器上使用 `Linux + Nginx + Python` 会更稳妥
- 数据量不大时，数据库继续用当前 `SQLite` 即可
- 图片/附件先存服务器本地目录，后续再迁移到阿里云 OSS
- 登录页、列表页、基础新增/删除功能已经可以先跑起来
...
# 2. 执行打包
python -m PyInstaller --noconfirm --clean --onefile --windowed --icon douyu_icon.ico --name "速维电脑租赁管理系统" main.py
```
打包后的文件位于：
```text
dist/速维电脑租赁管理系统.exe
...
python test_rental_ai_functional.py
python -m compileall core modules test_sql_migration.py
python -m PyInstaller --noconfirm --clean --onefile --windowed --icon douyu_icon.ico --name "豆宇科技电脑租赁管理系统" main.py
```
本版本已验证：
- SQLite Migration / JSON 导入 / 版本回溯测试通过
- 全模块集成测试通过
- 租赁管理与 AI 助手功能测试通过
- PyInstaller 成功生成 `dist/速维电脑租赁管理系统.exe`
---
## 💾 数据管理
...
| 2 | `_migration_002_hardware_models` | 硬件型号库表：hardware_models（含规格、参考价格/月租、发布年份） |
| 3 | `_migration_003_sync_models` | 同步型号库：补充二手/老平台常用型号，更新参考价格 |
