...
export function returnPeriodSettlement(input: {
  anchorDate: string;
  returnDate: string;
  bills: ReturnSettlementBill[];
}) {
  ...
  const periodDates = (periodNo: number) => {
    const period = billingPeriod(input.anchorDate, periodNo);
    return { periodStart: period.start, periodEnd: period.displayEnd };
  };
  ...
  for (const bill of rentBills) {
    ...
    const paidPeriods =
      periodAmountCents > 0
        ? Math.min(
            range.span,
            Math.floor((paidCents + 0.5) / periodAmountCents),
          )
        : 0;
    for (let periodNo = range.start; periodNo <= range.end; periodNo += 1) {
      ...
      if (periodNo <= currentPeriod.periodNo && !settled) {
        const dates =
          periodNo === currentPeriod.periodNo
            ? {
                periodStart: currentPeriod.start,
                periodEnd: currentPeriod.displayEnd,
              }
            : periodDates(periodNo);
        unpaidPeriods.push({
          periodNo,
          ...dates,
          amount: Math.round(periodAmountCents) / 100,
          kind: periodNo < currentPeriod.periodNo ? "historical" : "current",
        });
      }
      if (periodNo < currentPeriod.periodNo && !settled) {
  ...
  return {
    ...
    unpaidPeriods: unpaidPeriods.sort((a, b) => a.periodNo - b.periodNo),
  };
}
...
export function calculateReturnRent(input: {
  ...
  const elapsed =
    Math.ceil(
      (Date.parse(`${input.returnDate}T00:00:00+08:00`) -
        Date.parse(`${input.periodStart}T00:00:00+08:00`)) /
        DAY_MS,
    ) + 1;
  ...
  const charge =
    input.mode === "full_month"
      ? full
      : input.mode === "daily"
        ? Math.min(full, Math.round((full * usedDays) / 30))
        : 0;
  return {
    usedDays,
    remainingDays,
