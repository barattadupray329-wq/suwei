export type AllocationBill = { id: number; amount: string | number; paidAmount: string | number; waivedAmount?: string | number; dueDate: string }
...
export const billOutstandingCents = (bill: Pick<AllocationBill, 'amount' | 'paidAmount' | 'waivedAmount'>) => Math.max(0, moneyToCents(bill.amount) - moneyToCents(bill.paidAmount) - moneyToCents(bill.waivedAmount ?? 0))
