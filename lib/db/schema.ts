...
export const adminApplications = sqliteTable('admin_applications', {
  id: integer('id').primaryKey({ autoIncrement: true }), name: text('name').notNull(), email: text('email').notNull(), phone: text('phone').notNull(), passwordHash: text('passwordHash').notNull(), status: text('status').notNull().default('pending'), reviewedBy: text('reviewedBy'), reviewedAt: timestampMs('reviewedAt'), rejectionReason: text('rejectionReason'), createdAt: timestampMs('createdAt').notNull().default(now), updatedAt: timestampMs('updatedAt').notNull().default(now),
})
