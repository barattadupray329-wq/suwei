...
export const receivableBills = sqliteTable('receivable_bills', {
  id: integer('id').primaryKey({ autoIncrement: true }), userId: text('userId').notNull(), rentalId: integer('rentalId').notNull(), renewalRecordId: integer('renewalRecordId'), billNo: text('billNo').notNull(), periodStart: text('periodStart').notNull(), periodEnd: text('periodEnd').notNull(), dueDate: text('dueDate').notNull(), billType: text('billType').notNull().default('租金'), amount: text('amount').notNull(), paidAmount: text('paidAmount').notNull().default('0'), status: text('status').notNull().default('待收'), reversedAt: integer('reversedAt', { mode: 'timestamp_ms' }), notes: text('notes'), createdAt: integer('createdAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`), updatedAt: integer('updatedAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`),
}, (table) => [unique().on(table.userId, table.billNo), index('receivable_bills_user_renewal_idx').on(table.userId, table.renewalRecordId)])
...
export const returnSettlements = sqliteTable('return_settlements', {
  id: integer('id').primaryKey({ autoIncrement: true }), userId: text('userId').notNull(), rentalId: integer('rentalId').notNull(), returnRecordId: integer('returnRecordId').notNull(), customerPhone: text('customerPhone').notNull(), calculatedRefund: text('calculatedRefund').notNull().default('0'), minimumTermMet: integer('minimumTermMet', { mode: 'boolean' }).notNull().default(false), finalRefund: text('finalRefund').notNull().default('0'), handlingType: text('handlingType').notNull(), refundStatus: text('refundStatus').notNull().default('无需退款'), refundMethod: text('refundMethod'), refundDate: text('refundDate'), reason: text('reason'), operatorName: text('operatorName').notNull(), createdAt: integer('createdAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`), updatedAt: integer('updatedAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`),
}, (table) => [unique().on(table.userId, table.returnRecordId), index('return_settlements_user_rental_idx').on(table.userId, table.rentalId)])

export const customerCreditLedger = sqliteTable('customer_credit_ledger', {
  id: integer('id').primaryKey({ autoIncrement: true }), userId: text('userId').notNull(), customerPhone: text('customerPhone').notNull(), sourceRentalId: integer('sourceRentalId').notNull(), returnSettlementId: integer('returnSettlementId'), entryType: text('entryType').notNull(), amount: text('amount').notNull(), entryDate: text('entryDate').notNull(), operatorName: text('operatorName').notNull(), notes: text('notes'), createdAt: integer('createdAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`),
}, (table) => [index('customer_credit_user_phone_idx').on(table.userId, table.customerPhone), index('customer_credit_user_rental_idx').on(table.userId, table.sourceRentalId)])
...
export const businessSettings = sqliteTable('business_settings', { id: integer('id').primaryKey({ autoIncrement: true }), userId: text('userId').notNull().unique(), storeName: text('storeName'), lessorType: text('lessorType').notNull().default('个人'), lessorName: text('lessorName').notNull().default(''), identityNo: text('identityNo'), contactName: text('contactName'), phone: text('phone'), address: text('address'), paymentInfo: text('paymentInfo'), contractTerms: text('contractTerms'), themeMode: text('themeMode').notNull().default('system'), themeColor: text('themeColor').notNull().default('green'), updatedAt: integer('updatedAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`) })
...
export const aiUsageDaily = sqliteTable('ai_usage_daily', {
  ...
  estimatedNeurons: integer('estimatedNeurons').notNull().default(0),
