...
export const receivableBills = sqliteTable('receivable_bills', {
  id: integer('id').primaryKey({ autoIncrement: true }), userId: text('userId').notNull(), rentalId: integer('rentalId').notNull(), billNo: text('billNo').notNull(), periodStart: text('periodStart').notNull(), periodEnd: text('periodEnd').notNull(), dueDate: text('dueDate').notNull(), billType: text('billType').notNull().default('租金'), amount: text('amount').notNull(), paidAmount: text('paidAmount').notNull().default('0'), waivedAmount: text('waivedAmount').notNull().default('0'), status: text('status').notNull().default('待收'), notes: text('notes'), createdAt: integer('createdAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`), updatedAt: integer('updatedAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`),
}, (table) => [unique().on(table.userId, table.billNo)])

export const waiverRecords = sqliteTable('waiver_records', {
  id: integer('id').primaryKey(), userId: text('userId').notNull(), rentalId: integer('rentalId').notNull(), billId: integer('billId').notNull(), clientRequestId: text('clientRequestId').notNull(), amount: text('amount').notNull(), waiverDate: text('waiverDate').notNull(), waiverType: text('waiverType').notNull(), reason: text('reason').notNull(), operatorUserId: text('operatorUserId').notNull(), operatorName: text('operatorName').notNull(), createdAt: integer('createdAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`),
}, (table) => [unique('waiver_records_user_request_bill_unique').on(table.userId, table.clientRequestId, table.billId), index('waiver_records_user_rental_idx').on(table.userId, table.rentalId)])
...
export const rentalItemPricePeriods = sqliteTable('rental_item_price_periods', {
  id: integer('id').primaryKey({ autoIncrement: true }), userId: text('userId').notNull(), rentalId: integer('rentalId').notNull(), rentalItemId: integer('rentalItemId').notNull(), sourceRentalItemId: integer('sourceRentalItemId').notNull(), deviceCodes: text('deviceCodes').notNull(), quantity: integer('quantity').notNull(), effectiveDate: text('effectiveDate').notNull(), previousMonthlyRent: text('previousMonthlyRent').notNull(), newMonthlyRent: text('newMonthlyRent').notNull(), proratedDifference: text('proratedDifference').notNull().default('0'), reason: text('reason').notNull(), operatorUserId: text('operatorUserId').notNull(), operatorName: text('operatorName').notNull(), createdAt: integer('createdAt', { mode: 'timestamp_ms' }).notNull().default(sql`(unixepoch() * 1000)`),
}, (table) => [index('rental_item_price_periods_user_rental_idx').on(table.userId, table.rentalId), index('rental_item_price_periods_item_effective_idx').on(table.rentalItemId, table.effectiveDate)])
