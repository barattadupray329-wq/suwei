...
export async function requestCustomerOtp(rawPhone: string, requestIp: string) {
  ...
  const [recent] = await db.select().from(customerOtpChallenges).where(and(eq(customerOtpChallenges.requestIpHash, ipHash), gt(customerOtpChallenges.createdAt, new Date(Date.now() - 60000)))).orderBy(desc(customerOtpChallenges.createdAt)).limit(1)
  if (recent) throw new Error('请求过于频繁，请一分钟后再试')
