...
| 模块 | 功能 | 状态 |
|------|------|------|
| `nutstore_sync.py` | 坚果云同步 | ✅ 完成 |
| `sync_server_manager.py` | HTTP 同步服务器 | ✅ 完成 |
| `server_discovery.py` | 服务器发现 | ✅ 完成 |
| `splash_screen.py` | 启动屏幕 | ✅ 完成 |
| `client_setup.py` | 客户端配置 | ✅ 完成 |
| `mode_selection.py` | 模式选择 | ✅ 完成 |
...
1. **核心逻辑** — 共享的业务逻辑和数据管理
2. **单机版** — 单台电脑使用的版本（本地存储，无网络同步）
3. **网络版** — 支持多端同步的网络版本（HTTP 同步 + 坚果云）
4. **局域网版** — 局域网同步版本（⏸️ 暂停开发）

### 开发约定

1. **默认修改核心逻辑**: 当用户指定修改某个版本时，实际上默认修改的是核心逻辑部分
2. **核心逻辑变更同步**: 核心逻辑的任何变动都需要同步到其他版本（单机版、网络版）
3. **版本差异**: 各版本之间的差异仅在于同步方式、界面配置和网络相关功能
...
✅ **阶段 2: 版本入口分离**
- [x] 创建 versions/ 目录结构
- [x] 创建单机版入口 (versions/standalone/)
- [x] 创建网络版入口 (versions/network/)
- [x] 创建局域网版占位 (versions/lan/)
...
# 单机版
python versions/standalone/main.py

# 网络版
python versions/network/main.py

# 局域网版 (预留)
python versions/lan/main.py
```
...
# 单机版打包
python -m PyInstaller --noconfirm --clean --onefile --windowed --name "速维租赁系统单机版" versions/standalone/main.py
```
