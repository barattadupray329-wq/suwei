...
describe("退租租金结算", () => {
  test("只处理退租所在的未收当期，已收当期不再处理", () => {
    ...
    const result = returnPeriodSettlement({
  ...
  test("历史未收期必须整期收，当前未收期才允许选择", () => {
    ...
    expect(result.unpaidPeriods).toEqual([
      { periodNo: 1, periodStart: "2026-06-10", periodEnd: "2026-07-09", amount: 100, kind: "historical" },
      { periodNo: 2, periodStart: "2026-07-10", periodEnd: "2026-08-09", amount: 100, kind: "current" },
    ]);
  });
  ...
  test("退剩余天数固定按30天折算且退款不超过实收", () => {
    expect(
      calculateReturnRent({ ...base, collectedAmount: 300, mode: "daily" }),
    ).toMatchObject({
      usedDays: 10,
      remainingDays: 20,
      chargeAmount: 100,
    ...
    expect(
      calculateReturnRent({ ...base, collectedAmount: 50, mode: "daily" })
        .refundAmount,
    ).toBe(0);
  });
  ...
  test("退本期全额只退本期实收", () => {
    expect(
      calculateReturnRent({ ...base, collectedAmount: 180, mode: "waive" }),
    ).toMatchObject({ chargeAmount: 0, refundAmount: 180, collectAmount: 0 });
  });
  ...
  test("退租日超出账期时剩余天数不为负数", () => {
    ...
    ).toMatchObject({ usedDays: 30, remainingDays: 0, refundAmount: 0 });
  });
  ...
  test("退租后的每一期都按剩余设备数量直接重算", () => {
    ...
    expect(
      ...
      {
        id: 5,
        previousAmountCents: 42000,
        nextAmountCents: 14000,
        reductionCents: 28000,
      },
      {
        id: 6,
        previousAmountCents: 42000,
  ...
  test("重算账单不会把应收调低到已收金额以下", () => {
    ...
    ).toMatchObject({ nextAmountCents: 20000, reductionCents: 22000 });
  });
});
