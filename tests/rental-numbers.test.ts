...
import { buildRentalNumbers, buildRentalNumberPreview, normalizeRentalDate } from '../lib/rental-numbers'

describe('租赁编号生成', () => {
  it('为空数据库生成首个合同与设备编号', () => {
    expect(buildRentalNumbers('2026-07-23', [{ deviceType: '台式机', quantity: 1 }])).toEqual({
  ...
  it('按同日已有编号递增并忽略异常编号', () => {
    ...
    expect(result).toEqual({ contractNo: 'HT20260723-003', deviceCodes: ['NB20260723-004'] })
  })
