...
from modules.funds_flow import FundsFlowFrame
...
from modules.inventory_management import InventoryManagementFrame
...
class MainWindow:
    ...
    def __init__(self, username: str, data_manager: DataManager, root: tk.Tk = None, content_frame: tk.Frame = None):
        ...
        if content_frame is not None:
            ...
            self.root.title("豆宇科技电脑租赁管理系统 V2")
    ...
    def _setup_styles(self):
        ...
        accent_soft = DarkTheme.lighten(accent, 26)
        ...
        style.map("Treeview", background=[("selected", DarkTheme.BG_HOVER)], foreground=[("selected", DarkTheme.TEXT_PRIMARY)])
    ...
    def _create_layout(self):
        """创建主布局 — 宽侧栏 + 顶部品牌栏"""
        self.content_frame = self._build_shell(self.root)
        self._switch_module("dashboard")

    def _create_layout_in_container(self):
        """在传入的容器中创建布局（单根窗口模式）"""
        self.content_frame = self._build_shell(self.content_frame)
    ...
    def _build_shell(self, parent):
        ...
        top_bar = tk.Frame(parent, bg=DarkTheme.BG_SECONDARY, height=56)
        top_bar.pack(fill=tk.X, side=tk.TOP)
        top_bar.pack_propagate(False)
        tk.Label(top_bar, text="豆宇科技", font=("微软雅黑", 17, "bold"), fg=DarkTheme.ACCENT_PRIMARY, bg=DarkTheme.BG_SECONDARY).pack(side=tk.LEFT, padx=(22, 2))
        tk.Label(top_bar, text="电脑租赁管理系统", font=("微软雅黑", 13), fg=DarkTheme.TEXT_SECONDARY, bg=DarkTheme.BG_SECONDARY).pack(side=tk.LEFT)
        tk.Label(top_bar, text=f"👤  {self.username}", font=DarkTheme.FONT_LABEL, fg=DarkTheme.TEXT_SECONDARY, bg=DarkTheme.BG_SECONDARY).pack(side=tk.RIGHT, padx=22)

        main_container = tk.Frame(parent, bg=DarkTheme.BG_PRIMARY)
        ...
        tk.Label(nav_hd, text="导 航", font=("微软雅黑", 13, "bold"), fg=DarkTheme.TEXT_PRIMARY, bg=DarkTheme.BG_SECONDARY).pack(side=tk.LEFT, padx=18, pady=24)
        tk.Frame(sidebar, bg=DarkTheme.BORDER_COLOR, height=1).pack(fill=tk.X, padx=14)

        self.nav_buttons = {}
        nav_items = [("📊  财务统计", "dashboard"), ("📒  资金流水", "funds_flow"), ("📋  租赁管理", "rental"), ("📦  库存管理", "inventory"), ("⏰  到期提醒", "reminder"), ("👥  用户管理", "user_mgmt"), ("⚙️  系统设置", "settings")]
        for text, key in nav_items:
            btn = ttk.Button(sidebar, text=text, style="Nav.TButton", command=lambda k=key: self._switch_module(k))
            btn.pack(fill=tk.X, padx=10, pady=3)
            self.nav_buttons[key] = btn

        tk.Frame(sidebar, bg=DarkTheme.BG_SECONDARY).pack(fill=tk.X, expand=True)
        tk.Frame(sidebar, bg=DarkTheme.BORDER_COLOR, height=1).pack(fill=tk.X, padx=14)
        logout_btn = tk.Button(sidebar, text="退 出 系 统", font=DarkTheme.FONT_BUTTON, fg=DarkTheme.TEXT_SECONDARY, bg=DarkTheme.BG_SECONDARY, relief=tk.FLAT, cursor="hand2", command=self._handle_logout)
        ...
        content_shell = tk.Frame(main_container, bg=DarkTheme.BG_PRIMARY)
        content_shell.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        content_canvas = tk.Canvas(content_shell, bg=DarkTheme.BG_PRIMARY, highlightthickness=0, bd=0)
        scrollbar = ttk.Scrollbar(content_shell, orient="vertical", command=content_canvas.yview)
        scrollable = tk.Frame(content_canvas, bg=DarkTheme.BG_PRIMARY)
        scroll_window = content_canvas.create_window((0, 0), window=scrollable, anchor="nw")
        ...
        def _update_scrollregion(event=None):
            ...
            content_canvas.itemconfigure(scroll_window, width=content_canvas.winfo_width())
        ...
        content_canvas.bind("<Configure>", _update_scrollregion)
        _bind_mousewheel(parent)

        self.content_canvas = content_canvas
        self.content_scrollbar = scrollbar
        self.content_area = scrollable

        self.status_bar = tk.Frame(parent, bg=DarkTheme.BG_CARD, height=30)
        self.status_bar.pack(fill=tk.X, side=tk.BOTTOM)
        self.status_bar.pack_propagate(False)
        self.status_label = tk.Label(self.status_bar, text="✓ 系统就绪", font=DarkTheme.FONT_SMALL, fg=DarkTheme.TEXT_MUTED, bg=DarkTheme.BG_CARD)
        self.status_label.pack(side=tk.LEFT, padx=16, pady=4)
        self.key_hint_label = tk.Label(self.status_bar, text="F5 刷新  ·  Ctrl+F 搜索  ·  鼠标滚轮可上下滚动", font=DarkTheme.FONT_SMALL, fg=DarkTheme.TEXT_MUTED, bg=DarkTheme.BG_CARD)
        self.key_hint_label.pack(side=tk.RIGHT, padx=16, pady=4)
        parent.bind("<F5>", lambda e: self._refresh_current())
        parent.bind("<Control-f>", lambda e: self._focus_search())
        self._enable_global_scroll(parent, content_canvas)
        self._fit_current_module = lambda: None
        return scrollable
    ...
    def _switch_module(self, module_key: str):
        ...
        for key, btn in self.nav_buttons.items():
            btn.configure(style="Nav.Active.TButton" if key == module_key else "Nav.TButton")
        if module_key == "dashboard":
            self.current_module = DashboardFrame(target_frame, self.data_manager)
        elif module_key == "funds_flow":
            self.current_module = FundsFlowFrame(target_frame, self.data_manager)
        elif module_key == "rental":
            self.current_module = RentalManagementFrame(target_frame, self)
        elif module_key == "inventory":
            self.current_module = InventoryManagementFrame(target_frame, self.data_manager)
        elif module_key == "reminder":
            self.current_module = DueReminderFrame(target_frame, self.data_manager)
        elif module_key == "user_mgmt":
            self.current_module = UserManagementFrame(target_frame, self.data_manager, self._get_user_role())
    ...
    def _enable_global_scroll(self, parent, content):
        """让滚轮始终滚动内容区。"""
        def _scroll(event):
            try:
                if event.delta:
        ...
        parent.bind_all('<MouseWheel>', _scroll, add='+')
        parent.bind_all('<Button-4>', _scroll, add='+')
        parent.bind_all('<Button-5>', _scroll, add='+')
