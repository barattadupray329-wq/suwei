...
const sections: Block[] = [
  {
    id: 'overview',
    title: '一、系统概述与登录',
    intro: '速维租赁管理面向门店操作员，分为对外官网、客户经理工作台和客户查询端三部分。登录后按账号权限显示可用功能。',
    cases: [
      { when: '客户经理要进入工作台', how: ['打开登录页，选择“账号密码”', '输入业务主管分配的账号和密码', '首次登录后建议尽快修改初始密码'], note: '客户经理账号由业务主管创建；账号被停用会立即退出登录。' },
  ...
  },
  {
    ...
    cases: [
      ...
      { when: '客户要换电脑或换配置', how: ['整台更换选择“客户要更换电脑或配置”', '仅配置变化选择“配置/租金变更”', '选择新租金生效账期并填写原因', '核对完整账期差额和变更前后配置再提交'], note: '租金只能从完整账期起始日调整；操作日处于账期中间时，从下一期生效，不按天拆分。' },
      ...
      { when: '合同录错了怎么办', how: ['无收款、无履约的测试合同可由业务主管删除', '已履约合同不得删除', '正式合同按实际办理退租、日期调整或资料变更', '金额录错使用冲正'], note: '业务链条完整可查，月底对账不会出现合同或款项凭空消失。' },
    ],
  },
  {
    id: 'finance',
  ...
  },
  {
    id: 'accounts',
    title: '七、账号与权限',
    intro: '业务主管可创建客户经理账号并按岗位分配功能权限，也可登记普通客户。',
    cases: [
      { when: '新增客户经理并授权', how: ['进入账号与权限', '填写姓名、登录账号、手机号和临时密码', '勾选该客户经理可使用的功能', '保存后安全告知客户经理临时密码'], note: '权限包括租赁操作、资金查看、合同管理、账号管理和系统设置。' },
      { when: '停用离职客户经理', how: ['在客户经理列表找到对应账号', '点击停用', '确认后账号立即退出登录'], note: '停用不会删除历史记录，业务仍可追溯。' },
    ],
  },
  {
    id: 'settings',
