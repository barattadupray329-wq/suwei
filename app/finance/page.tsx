...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const filters = { query: value('query'), type: value('type'), method: value('method'), from: value('from'), to: value('to'), view: value('view') === 'receipts' ? 'receipts' as const : undefined, page: Math.max(1, Number(value('page')) || 1), pageSize: 20 }
