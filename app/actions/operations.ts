...
import { accountLedger, auditLogs, customerCreditLedger, lossRecords, paymentRecords, receivableBills, rentalEvents, rentalItems, rentalOperations, rentals, returnRecords, returnSettlements } from '@/lib/db/schema'
...
const rentRefundHandlingSchema = z.enum(['none', 'credit', 'refund'])
export type ReturnInput = z.infer<typeof operationSchema> & { condition: '完好'|'轻微磨损'|'损坏'; deductionAmount: number; depositRefund: number; rentRefundAmount: number; rentRefundHandling: z.infer<typeof rentRefundHandlingSchema>; rentRefundReason: string; collectionSettlement: z.infer<typeof settlementSchema>; refundSettlement: z.infer<typeof settlementSchema> }
export type LossInput = z.infer<typeof operationSchema> & { unitCompensation: number }

const returnSchema = operationSchema.extend({ condition:z.enum(['完好','轻微磨损','损坏']),deductionAmount:z.number().nonnegative(),depositRefund:z.number().nonnegative(),rentRefundAmount:z.number().nonnegative(),rentRefundHandling:rentRefundHandlingSchema,rentRefundReason:z.string().trim().max(500),collectionSettlement:settlementSchema,refundSettlement:settlementSchema })
...
export async function returnRentalItems(input: ReturnInput[]) {
  ...
  const rows=values.map((value,index)=>{const item=byId.get(value.rentalItemId);if(!item)throw new Error('包含不存在的设备');dateOnly(value.date);if(item.startDate&&value.date<item.startDate)throw new Error(`${item.deviceName} 的退租日期不能早于起租日期`);const available=availableQuantity(item);if(value.quantity>available)throw new Error(`${item.deviceName} 最多可退 ${available} 台`);let calculatedRefundCents=0;if(item.endDate&&value.date<item.endDate){const days=Math.max(0,Math.ceil((dateOnly(item.endDate).getTime()-dateOnly(value.date).getTime())/86400000));calculatedRefundCents=Math.round(toCents(item.monthlyRent)*days*value.quantity/30)}const minimumTermMet=rental.billingType==='daily'||value.date>=rental.endDate;const finalRefundCents=toCents(value.rentRefundAmount);if(finalRefundCents>calculatedRefundCents)throw new Error(`${item.deviceName} 的租金退款不能超过按天试算金额 ${fromCents(calculatedRefundCents)} 元`);if(value.rentRefundHandling==='none'&&finalRefundCents!==0)throw new Error('选择不退租金时，退款金额必须为 0');if(value.rentRefundHandling!=='none'&&finalRefundCents<=0)throw new Error('请选择有效的租金退款金额');if((!minimumTermMet||finalRefundCents!==calculatedRefundCents)&&!value.rentRefundReason.trim())throw new Error('最低租期内退款或手工调整金额时必须填写协商说明');return{value,item,available,calculatedRefundCents,minimumTermMet,finalRefundCents,id:Date.now()*1000+index}})
  ...
  const deductionCents=rows.reduce((sum,row)=>sum+toCents(row.value.deductionAmount),0),rentRefundCents=rows.reduce((sum,row)=>sum+row.finalRefundCents,0),collectedCents=rows.reduce((sum,row)=>sum+(row.value.collectionSettlement.timing==='now'?toCents(row.value.deductionAmount):0),0)
  const totalCents=toCents(rental.totalRent)+deductionCents-rentRefundCents,paidCents=toCents(rental.paidAmount)+collectedCents
  if(totalCents<0)throw new Error('退租调整后合同总额不能小于 0')
  const statements:Array<Parameters<typeof db.batch>[0][number]>=[]
  for(const row of rows){const v=row.value,next=row.item.returnedQuantity+v.quantity,collected=v.collectionSettlement.timing==='now'?v.deductionAmount:0,operationNo=`${operationNumber('return',rentalId)}-${row.id}`;statements.push(db.insert(rentalOperations).values({userId,rentalId,operationNo,operationType:'return',status:'completed',idempotencyKey:operationIdempotencyKey({userId,rentalId,type:'return',clientRequestId:crypto.randomUUID()}),actorUserId:actorId,actorName:name,summary:`${rental.contractNo} 退租 ${row.item.deviceName} ${v.quantity} 台`,completedAt:new Date()}),db.update(rentalItems).set({returnedQuantity:next,updatedAt:new Date()}).where(and(eq(rentalItems.userId,userId),eq(rentalItems.id,row.item.id))),db.insert(returnRecords).values({id:row.id,userId,rentalId,rentalItemId:row.item.id,quantity:v.quantity,returnDate:v.date,condition:v.condition,deductionAmount:fromCents(toCents(v.deductionAmount)),depositRefund:fromCents(toCents(v.depositRefund)),notes:v.notes,operatorName:name}),db.insert(rentalEvents).values({userId,rentalId,itemId:row.item.id,eventType:'退租',status:'已完成',eventDate:v.date,beforeSnapshot:{availableQuantity:row.available},afterSnapshot:{availableQuantity:row.available-v.quantity,returnedQuantity:next},feeAdjustment:fromCents(toCents(v.deductionAmount)-toCents(v.depositRefund)),operatorName:name,notes:v.notes}),db.insert(auditLogs).values({userId,actorUserId:actorId,actorName:name,action:'办理退租',resourceType:'租赁合同',resourceId:String(rentalId),summary:`${rental.contractNo} 退租 ${row.item.deviceName} ${v.quantity} 台`,metadata:{rentalItemId:row.item.id,quantity:v.quantity}}));statements.push(db.insert(returnSettlements).values({id:row.id,userId,rentalId,returnRecordId:row.id,customerPhone:rental.customerPhone,calculatedRefund:fromCents(row.calculatedRefundCents),minimumTermMet:row.minimumTermMet,finalRefund:fromCents(row.finalRefundCents),handlingType:v.rentRefundHandling==='none'?'不退':v.rentRefundHandling==='credit'?'转客户余额':'直接退款',refundStatus:v.rentRefundHandling==='refund'?(v.refundSettlement.timing==='now'?'已退款':'待退款'):'无需退款',refundMethod:v.rentRefundHandling==='refund'?v.refundSettlement.method:null,refundDate:v.rentRefundHandling==='refund'&&v.refundSettlement.timing==='now'?v.date:null,reason:v.rentRefundReason||null,operatorName:name}));if(v.rentRefundHandling==='credit'&&row.finalRefundCents>0)statements.push(db.insert(customerCreditLedger).values({userId,customerPhone:rental.customerPhone,sourceRentalId:rentalId,returnSettlementId:row.id,entryType:'退租转入',amount:fromCents(row.finalRefundCents),entryDate:v.date,operatorName:name,notes:v.rentRefundReason||`${rental.contractNo} 退租转客户余额`}));if(v.rentRefundHandling==='refund'&&row.finalRefundCents>0)statements.push(db.insert(accountLedger).values({userId,rentalId,entryType:v.refundSettlement.timing==='now'?'租金退款':'租金待退',amount:fromCents(-row.finalRefundCents),entryDate:v.date,operatorName:name,notes:`${v.refundSettlement.timing==='now'?`已通过${v.refundSettlement.method}退款`:'约定以后退款'}${v.rentRefundReason?`；${v.rentRefundReason}`:''}`}));if(row.finalRefundCents>0)statements.push(db.insert(receivableBills).values({userId,rentalId,billNo:`RETURN-${rentalId}-${row.id}`,periodStart:v.date,periodEnd:v.date,dueDate:v.date,billType:'提前退租减免',amount:fromCents(-row.finalRefundCents),paidAmount:'0.00',status:'已调整',notes:'提前退租按实际使用天数结算'}));if(v.deductionAmount>0)statements.push(db.insert(receivableBills).values({userId,rentalId,billNo:`RETURN-CHARGE-${rentalId}-${row.id}`,periodStart:v.date,periodEnd:v.date,dueDate:v.date,billType:'退租赔偿',amount:fromCents(toCents(v.deductionAmount)),paidAmount:fromCents(toCents(collected)),status:collected>0?'已结清':'待收',notes:`${row.item.deviceName} 退租赔偿`}));if(collected>0)statements.push(db.insert(paymentRecords).values({userId,rentalId,returnRecordId:row.id,amount:fromCents(toCents(collected)),paymentDate:v.date,paymentMethod:v.collectionSettlement.method,feeType:'其他',operatorName:name,notes:'退租赔偿即时收款'}));if(v.depositRefund>0)statements.push(db.insert(accountLedger).values({userId,rentalId,entryType:v.refundSettlement.timing==='now'?'押金退还':'押金待退',amount:fromCents(-toCents(v.depositRefund)),entryDate:v.date,operatorName:name,notes:v.notes}))}
