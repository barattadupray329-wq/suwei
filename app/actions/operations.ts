...
export type ReturnInput = z.infer<typeof operationSchema> & { condition: '完好'|'轻微磨损'|'损坏'; deductionAmount: number; depositRefund: number; billingMode: z.infer<typeof returnBillingModeSchema>; billingReason: string; collectionSettlement: z.infer<typeof settlementSchema>; refundSettlement: z.infer<typeof settlementSchema> }
export type LossInput = z.infer<typeof operationSchema> & { unitCompensation: number }

const returnSchema = operationSchema.extend({ condition:z.enum(['完好','轻微磨损','损坏']),deductionAmount:z.number().nonnegative(),depositRefund:z.number().nonnegative(),billingMode:returnBillingModeSchema,billingReason:z.string().trim().max(500),collectionSettlement:settlementSchema,refundSettlement:settlementSchema })
...
export async function returnRentalItems(input: ReturnInput[]) {
  ...
  const totalCents=toCents(rental.totalRent)+deductionCents-billingAdjustmentCents,paidCents=toCents(rental.paidAmount)+collectedCents
  ...
  for (const row of rows) {
    ...
    const modeLabel = row.value.billingMode === 'daily' ? `按 30 天折算，计 ${row.billing.usedDays} 天` : '本期不收'
