...
export async function returnRentalItems(input: ReturnInput[]) {
  ...
  const billRecalculations=rows.flatMap((row)=>recalculateBillsAfterReturn({bills,monthlyRent:row.item.monthlyRent,returnedQuantity:row.value.quantity,returnDate:row.value.date}))
  ...
  const totalCents=adjustedRentCents+deductionCents,paidCents=toCents(rental.paidAmount)+collectedCents
