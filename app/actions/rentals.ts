...
export async function getRentalPage(input: RentalListQuery = {}) {
  ...
  // 未结清账单存在时取其中应收日 dueDate 最早的一笔（最紧迫该收的欠款；不能用 periodEnd，
  // 那是账单覆盖的服务周期结束日，会比 dueDate 晚一整月，导致早就该收的欠款被显示成"还没到期"，
  // 跟仪表盘"逾期待收"里按 dueDate 统计的口径自相矛盾）；全部结清时才回退到合同/设备明细/账单
  // 里最晚的日期。排序必须用同一套口径的日期，否则列表顺序和"N天后到期"文案会对不上。
  ...
  const effectiveEndDateSql = sql<string>`coalesce(
    (select min(b.dueDate) from receivable_bills b where b.userId = ${rentals.userId} and b.rentalId = ${rentals.id} and ${unpaidBillCondition}),
    (select max(d) from (
  ...
  const [[summaryRow], rows] = await Promise.all([
    ...
    db.select({ id: rentals.id, orderType: rentals.orderType, lifecycleStatus: rentals.lifecycleStatus, deletedAt: rentals.deletedAt, contractNo: rentals.contractNo, customerCompany: rentals.customerCompany, customerName: rentals.customerName, customerPhone: rentals.customerPhone, deviceName: rentals.deviceName, quantity: rentals.quantity, billingType: rentals.billingType, startDate: rentals.startDate, endDate: rentals.endDate, totalRent: rentals.totalRent, paidAmount: rentals.paidAmount, outstandingAmount: sql<number>`${outstandingAmount}`, overdueOutstandingAmount: sql<number>`${overdueOutstandingAmount}`, paymentStatus: rentals.paymentStatus, status: rentals.status, assigneeName: rentals.assigneeName, headerRemark: rentals.headerRemark, createdAt: rentals.createdAt }).from(rentals).where(where).orderBy(...orderBy).limit(value.pageSize).offset(offset),
  ])
  ...
  const normalizedRows = rows.map((row) => {
    ...
    // "合同/设备/账单中最晚的日期"，用来提醒即将到期需要续租。
    const outstandingBills = bills.filter((bill) => Number(bill.paidAmount) < Number(bill.amount))
    const earliestOutstandingDueDate = outstandingBills.map((bill) => bill.dueDate).filter(Boolean).sort().at(0) ?? null
    const effectiveEndDate = earliestOutstandingDueDate
...
async function createRentalOperation(input: RentalInput, orderType: RentalOrderType, initialCollection?: InitialCollectionInput) {
  ...
  // 合同要等到次日 01:00 定时任务才会长出账单。这里只扫这一张��合同、且用不抛错版本，失败也不影响
  // 合同创建本身（大不了等定时任务兜底）。
...
async function updateDraftOperation(rentalId: number, input: RentalInput) {
  ...
  const expectedEndDate = rentalEndDate(value.startDate, value.duration, value.billingType)
  if (value.endDate !== expectedEndDate) throw new Error('到期日期与计费方式、起租日期或租赁时间不一致')
  const contractNo = rental.contractNo || `CG-${Date.now()}`
  const stamp = contractNo.replace(/^CG-/, '')
  const normalizedItems = value.items.map((item, index) => ({
    ...item,
    deviceName: normalizeDeviceName(item.deviceType, item.deviceName),
    deviceCode: `CG-SB-${stamp}-${index + 1}`,
    totalRent: Math.round(item.quantity * item.monthlyRent * value.duration * 100) / 100,
  }))
  ...
  const monthlyRent = normalizedItems.reduce((sum, item) => sum + item.monthlyRent * item.quantity, 0)
  const totalRent = normalizedItems.reduce((sum, item) => sum + item.totalRent, 0)
  const notes = [`计费方式：${value.billingType === 'daily' ? '日租' : '月租'}；租赁时间：${value.duration}${value.billingType === 'daily' ? '天' : '个月'}`, value.notes?.trim()].filter(Boolean).join('\n')
  const statements: Array<Parameters<typeof db.batch>[0][number]> = [
    db.update(rentals).set({
      assignedEmployeeId: assignee.id, assigneeUserId: assignee.id, assigneeName: assignee.name,
      customerCompany: value.customerCompany?.trim() || null, customerName: value.customerName, customerPhone: value.customerPhone, customerAddress: value.customerAddress,
      startDate: value.startDate, startDateReason, endDate: value.endDate, billingType: value.billingType, duration: value.duration, deposit: String(value.deposit), notes,
      deviceName: normalizedItems.map((item) => item.deviceName).join('、'), deviceType: normalizedItems.length > 1 ? '多设备' : first.deviceType, deviceCode: first.deviceCode, deviceConfig: first.deviceConfig,
      quantity, monthlyRent: String(monthlyRent), totalRent: String(totalRent), updatedAt: new Date(),
    }).where(and(eq(rentals.id, rentalId), eq(rentals.userId, userId))),
    db.delete(rentalItems).where(and(eq(rentalItems.rentalId, rentalId), eq(rentalItems.userId, userId))),
    ...buildChunkedInserts(rentalItems, normalizedItems.map((item) => ({ ...item, userId, rentalId, startDate: value.startDate, endDate: value.endDate, monthlyRent: String(item.monthlyRent), totalRent: String(item.totalRent) }))),
    db.insert(auditLogs).values({ userId, actorUserId: access.actorId, actorName: access.actorName, action: '修改', resourceType: '租赁合同', resourceId: String(rentalId), summary: `编辑草稿 ${contractNo}（${value.customerCompany || value.customerName}）`, metadata: { totalRent, quantity } }),
  ]
  await db.batch(statements as [typeof statements[number], ...Array<typeof statements[number]>])
  revalidatePath('/')
  revalidatePath('/rentals')
...
export async function updateDraft(rentalId: number, input: RentalInput) {
  return toActionResult('保存草稿修改', () => updateDraftOperation(rentalId, input))
}
...
export async function changeRentFromPeriod(input: PeriodRentChangeInput) {
  ...
  for (const bill of rentBills) {
    ...
    if (toCents(bill.paidAmount) === 0) {
      ...
      if (deltaCents < 0) statements.push(db.insert(customerCreditLedger).values({ userId, customerPhone: rental.customerPhone, sourceRentalId: rental.id, entryType: '调租余额', amount: fromCents(-deltaCents), entryDate: eventDate, operatorName: access.actorName, notes: `账单 ${bill.billNo} 降价形成客户余额，可抵扣或退款` }))
    }
  }
...
export async function correctRenewalPrice(input: RenewalCorrectionInput) {
  ...
  assertOfficialRental(rental)
  // 价格更正不再"另外生成一张续租补差/续租减免账单叠加在原账单上"——本店的规则是优惠只能跟着收款走，
  // 账单本身不存在脱离收款的"减免"概念。所以这里直接找到这次续租当时生成的"续租费"账单（本期 + 尚未
  // 到账的后续账期），把它们的应收金额本身改成正确单价对应的金额；如果某期已经全额收过款，多收的部分
  // 记入客户余额（customerCreditLedger），可供以后抵扣或退款，而不是留一条容易让人误解的"已抵扣"账单。
  const [[latest], immediateBills, futureBills] = await Promise.all([
    db.select().from(renewalAdjustments).where(and(eq(renewalAdjustments.userId, userId), eq(renewalAdjustments.renewalRecordId, renewal.id))).orderBy(desc(renewalAdjustments.createdAt), desc(renewalAdjustments.id)).limit(1),
    db.select().from(receivableBills).where(and(
      eq(receivableBills.userId, userId),
      eq(receivableBills.rentalId, renewal.rentalId),
      eq(receivableBills.renewalRecordId, renewal.id),
      eq(receivableBills.billType, '续租费'),
      ne(receivableBills.status, '已冲正'),
    )).orderBy(receivableBills.periodStart),
    db.select().from(receivableBills).where(and(
      eq(receivableBills.userId, userId),
      eq(receivableBills.rentalId, renewal.rentalId),
      gte(receivableBills.periodStart, renewal.newEndDate),
      ne(receivableBills.status, '已冲正'),
    )).orderBy(receivableBills.periodStart),
  ])
  if (!immediateBills.length) throw new Error('未找到该续租对应的账单，可能已被冲正，无法更正价格')
  const previousUnitPrice = Number(latest?.correctedUnitPrice ?? renewal.unitPrice ?? renewal.newMonthlyRent)
  if (toCents(previousUnitPrice) === toCents(value.correctedUnitPrice)) throw new Error('正确价格与当前有效价格相同，无需更正')
  const correctionDate = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit' }).format(new Date())
  const now = new Date()
  const newPerBillAmountCents = renewal.quantity * toCents(value.correctedUnitPrice)
  const recurringDeltaCents = (toCents(value.correctedUnitPrice) - toCents(previousUnitPrice)) * renewal.quantity
  const recurringBills = futureBills.filter((bill) => bill.billType.includes('续租租金') || bill.billType === '续租费')

  const statements: Array<Parameters<typeof db.batch>[0][number]> = []
  let immediateDifferenceCents = 0
  const affectedImmediateBillIds: number[] = []
  for (const bill of immediateBills) {
    const deltaCents = newPerBillAmountCents - toCents(bill.amount)
    if (!deltaCents) continue
    immediateDifferenceCents += deltaCents
    affectedImmediateBillIds.push(bill.id)
    const paidCents = toCents(bill.paidAmount)
    const overpaidCents = paidCents - newPerBillAmountCents
    const newStatus = paidCents >= newPerBillAmountCents ? '已结清' : paidCents > 0 ? '部分收款' : '待收'
    statements.push(db.update(receivableBills).set({ amount: fromCents(newPerBillAmountCents), status: newStatus, notes: `${bill.notes ?? ''}${bill.notes ? '；' : ''}价格更正：${value.reason}`, updatedAt: now }).where(and(eq(receivableBills.userId, userId), eq(receivableBills.id, bill.id))))
    if (overpaidCents > 0) statements.push(db.insert(customerCreditLedger).values({ userId, customerPhone: rental.customerPhone, sourceRentalId: rental.id, entryType: '调价余额', amount: fromCents(overpaidCents), entryDate: correctionDate, operatorName: access.actorName, notes: `账单 ${bill.billNo} 价格更正后多收 ${fromCents(overpaidCents)} 元，形成客户余额，可抵扣或退款` }))
  }
  const recurringDifferenceCents = recurringDeltaCents * recurringBills.length
  const totalDifferenceCents = immediateDifferenceCents + recurringDifferenceCents
  if (!totalDifferenceCents) throw new Error('正确价格与当前有效价格相同，无需更正')
  const newTotalCents = toCents(rental.totalRent) + totalDifferenceCents
  if (newTotalCents < 0) throw new Error('更正后合同总额不能小于 0')
  const paymentStatus = paymentStatusFromCents(newTotalCents, toCents(rental.paidAmount))
  const previousAmount = Number(latest?.correctedAmount ?? renewal.renewalAmount)
  const correctedAmount = Number(fromCents(toCents(previousAmount) + immediateDifferenceCents))

  statements.push(
    db.insert(renewalAdjustments).values({ userId, rentalId: renewal.rentalId, renewalRecordId: renewal.id, previousUnitPrice: fromCents(toCents(previousUnitPrice)), correctedUnitPrice: fromCents(toCents(value.correctedUnitPrice)), previousAmount: fromCents(toCents(previousAmount)), correctedAmount: fromCents(toCents(correctedAmount)), differenceAmount: fromCents(immediateDifferenceCents), reason: value.reason, operatorUserId: access.actorId, operatorName: access.actorName }),
    db.update(rentalItems).set({ monthlyRent: fromCents(toCents(value.correctedUnitPrice)), updatedAt: now }).where(and(eq(rentalItems.userId, userId), eq(rentalItems.id, renewal.renewedRentalItemId))),
    db.insert(rentalEvents).values({ userId, rentalId: renewal.rentalId, itemId: renewal.renewedRentalItemId, eventType: '续租价格更正', status: '已完成', eventDate: correctionDate, beforeSnapshot: { unitPrice: previousUnitPrice, amount: previousAmount }, afterSnapshot: { unitPrice: value.correctedUnitPrice, amount: correctedAmount, correctedBillIds: affectedImmediateBillIds, propagatedBillIds: recurringBills.map((bill) => bill.id) }, reason: value.reason, feeAdjustment: fromCents(totalDifferenceCents), operatorName: access.actorName, notes: `直接更正本期账单金额；本期及以后账期统一调价，已传播 ${recurringBills.length} 期` }),
    db.update(rentals).set({ monthlyRent: fromCents(toCents(rental.monthlyRent) + recurringDeltaCents), totalRent: fromCents(newTotalCents), paymentStatus, updatedAt: now }).where(and(eq(rentals.id, renewal.rentalId), eq(rentals.userId, userId))),
    db.insert(auditLogs).values({ userId, actorUserId: access.actorId, actorName: access.actorName, action: '更正续租价格', resourceType: '续租记录', resourceId: String(renewal.id), summary: `${rental.contractNo} 续租单价 ${previousUnitPrice.toFixed(2)} 元更正为 ${value.correctedUnitPrice.toFixed(2)} 元，并传播至以后 ${recurringBills.length} 期`, metadata: { rentalId: renewal.rentalId, previousUnitPrice, correctedUnitPrice: value.correctedUnitPrice, previousAmount, correctedAmount, differenceAmount: fromCents(immediateDifferenceCents), recurringDifference: fromCents(recurringDifferenceCents), correctedBillIds: affectedImmediateBillIds, propagatedBillIds: recurringBills.map((bill) => bill.id), reason: value.reason } }),
  )
  for (const bill of recurringBills) {
    const newAmountCents = toCents(bill.amount) + recurringDeltaCents
    const paidCents = toCents(bill.paidAmount)
    const overpaidCents = paidCents - newAmountCents
    const newStatus = paidCents >= newAmountCents ? '已结清' : paidCents > 0 ? '部分收款' : '待收'
    statements.push(db.update(receivableBills).set({ amount: fromCents(newAmountCents), status: newStatus, notes: `${bill.notes ?? ''}${bill.notes ? '；' : ''}续租单价追溯调整：${value.reason}`, updatedAt: now }).where(and(eq(receivableBills.userId, userId), eq(receivableBills.id, bill.id))))
    if (overpaidCents > 0) statements.push(db.insert(customerCreditLedger).values({ userId, customerPhone: rental.customerPhone, sourceRentalId: rental.id, entryType: '调价余额', amount: fromCents(overpaidCents), entryDate: correctionDate, operatorName: access.actorName, notes: `账单 ${bill.billNo} 降价形成客户余额，可抵扣或退款` }))
  }
  await db.batch(statements as [typeof statements[number], ...Array<typeof statements[number]>])
  revalidatePath('/')
  revalidatePath('/rentals')
...
const paymentSchema = z.object({ amount: z.number().nonnegative(), discountAmount: z.number().nonnegative().default(0), paymentDate: z.string().min(1), paymentMethod: z.enum(['现金', '微信', '支付宝', '银行卡', '其他']), feeType: z.enum(['原合同租金', '续租费', '押金', '买断费', '其他']), billId: z.number().int().positive().optional(), renewalRecordId: z.number().int().positive().optional(), notes: z.string().optional() }).superRefine((value, context) => { if (value.amount <= 0 && value.discountAmount <= 0) context.addIssue({ code: 'custom', path: ['amount'], message: '实收与优惠减免不能同时为 0' }); if (value.feeType === '押金' && value.amount <= 0) context.addIssue({ code: 'custom', path: ['amount'], message: '押金收取金额必须大于 0' }); if (value.feeType === '押金' && value.discountAmount > 0) context.addIssue({ code: 'custom', path: ['discountAmount'], message: '押金收取不能使用优惠' }); if (value.discountAmount > 0 && (value.notes?.trim().length ?? 0) < 2) context.addIssue({ code: 'custom', path: ['notes'], message: '有优惠时请填写至少 2 个字的优惠原因' }) })
export type PaymentInput = z.infer<typeof paymentSchema>

export async function collectPayment(id: number, input: PaymentInput) {
  const userId = await getUserId()
  const value = paymentSchema.parse(input)
  await ensureOverdueRentBills(userId, value.paymentDate, id)
...
export async function reversePayment(paymentId: number, reason: string) {
  ...
  if (bills.length !== billIds.length) throw new Error('原收��的账单分配记录不完整，禁止冲正')
  const allocationsByBill = new Map<number, typeof allocations>()
  for (const allocation of allocations) {
    const bucket = allocationsByBill.get(allocation.billId)
    if (bucket) bucket.push(allocation)
...
export async function buyoutRentalItems(input: BuyoutBatchInput[], settlementInput: SettlementInput, forgiveExcessRent = true) {
  ...
  const finalItems=items.map((item)=>{const row=rows.find((r)=>r.item.id===item.id);return row?{...item,boughtOutQuantity:item.boughtOutQuantity+row.value.quantity}:item}),amountCents=rows.reduce((s,r)=>s+r.amountCents,0),statements:Array<Parameters<typeof db.batch>[0][number]>=[]
  // 买断设备后，尚未收款的后续月租账单按剩余设备数量重算；已收账单不追溯修改。disposals 必须叠加全部历史退租/丢失/买断记录，避免第二次及以后的处置漏算之前的处置。
  ...
  // 开关开启时（默认），免除"横跨���断日"的未收租金账单：账期从买断日之前开始、却延续到买断日之后
...
export async function moveRentalToTrash(input: number | z.input<typeof trashRentalSchema>, legacyReason = '测试或草稿数据清理') {
  ...
  const statements: Array<Parameters<typeof db.batch>[0][number]> = []
  if (rental.orderType === 'official') {
    // 删除录错正��订单：连带撤销本单的续租/退租/买断/报损/维修/变更及其收款、账务与账单。
