...
const changeSchema = z.object({
  ...
  deviceName: z.string().trim().min(2), deviceCode: optionalText, deviceConfig: optionalText, cpu: optionalText, motherboard: optionalText, memory: optionalText, storage: optionalText, graphicsCard: optionalText, powerSupply: optionalText, caseModel: optionalText, monitorInfo: optionalText, accessories: optionalText,
...
const snapshotKeys = ['deviceName','deviceType','deviceCode','deviceConfig','quantity','monthlyRent','totalRent','cpu','motherboard','memory','storage','graphicsCard','powerSupply','caseModel','monitorInfo','accessories'] as const
...
export async function changeRentalItem(input: RentalChangeInput) {
  ...
  await db.transaction(async tx => {
    ...
    const after = { ...snapshot(item), deviceName:value.deviceName, deviceCode:value.deviceCode||null, deviceConfig:value.deviceConfig||null, cpu:value.cpu||null, motherboard:value.motherboard||null, memory:value.memory||null, storage:value.storage||null, graphicsCard:value.graphicsCard||null, powerSupply:value.powerSupply||null, caseModel:value.caseModel||null, monitorInfo:value.monitorInfo||null, accessories:value.accessories||null, monthlyRent:String(value.monthlyRent), totalRent:String(value.totalRent) }
    ...
    await tx.update(rentals).set({ deviceName:items.map(current=>current.deviceName).join('、'),monthlyRent:String(monthlyRent),totalRent:String(totalRent),paymentStatus,updatedAt:new Date() }).where(and(eq(rentals.userId,userId),eq(rentals.id,value.rentalId)))
