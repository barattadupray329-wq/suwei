...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { account?: unknown; code?: unknown; newPassword?: unknown; confirmPassword?: unknown }
    ...
    if (!profile?.phone || !/^1\d{10}$/.test(profile.phone)) throw new CustomerOtpError('账号、验证码或绑定手机号不正确', 400)
    await verifyStaffPasswordOtp(profile.phone, String(body.code ?? ''))
    ...
    if (ownerId) await db.insert(auditLogs).values({ userId: ownerId, actorUserId: profile.id, actorName: profile.name, action: '短信验证修改密码', resourceType: '账号', resourceId: profile.id, summary: `${profile.name}通过绑定手机号验证修改登录密码`, metadata: {} })
