...
export async function GET() {
  try {
    ...
    const [rentalState, itemState, eventState, ledgerState, overdueState] = await Promise.all([
    ...
    const state = [rentalState[0], itemState[0], eventState[0], ledgerState[0]]
      ...
      .join('|')
    return NextResponse.json({ version: BUILD_VERSION, state, overdueReceivable: Number(overdueState[0]?.total ?? 0) }, { headers: { 'Cache-Control': 'no-store, max-age=0' } })
  } catch {
