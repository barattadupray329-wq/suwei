...
export async function GET() {
  try {
    ...
    const [rentalState, itemState, eventState, ledgerState, receivableState] = await Promise.all([
      ...
      db.select({
        overdue: sql<number>`coalesce(sum(case when ${rentals.endDate} < date('now', '+8 hours') and ${rentals.status} not in ('买断', '已买断', '已退租', '已退回', '已结束', '已关闭', '已完成', '丢失') and round(cast(${rentals.totalRent} as real) - cast(${rentals.paidAmount} as real), 2) > 0 then round(cast(${rentals.totalRent} as real) - cast(${rentals.paidAmount} as real), 2) else 0 end), 0)`,
        outstanding: sql<number>`coalesce(sum(case when round(cast(${rentals.totalRent} as real) - cast(${rentals.paidAmount} as real), 2) > 0 then round(cast(${rentals.totalRent} as real) - cast(${rentals.paidAmount} as real), 2) else 0 end), 0)`,
      }).from(rentals).where(and(eq(rentals.userId, access.userId), eq(rentals.orderType, 'official'), eq(rentals.lifecycleStatus, 'active'))),
    ])
    const state = [rentalState[0], itemState[0], eventState[0], ledgerState[0]]
      ...
      .join('|')
    return NextResponse.json({ version: BUILD_VERSION, state, overdueReceivable: Number(receivableState[0]?.overdue ?? 0), outstandingReceivable: Number(receivableState[0]?.outstanding ?? 0) }, { headers: { 'Cache-Control': 'no-store, max-age=0' } })
  } catch {
