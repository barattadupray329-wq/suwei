...
export async function GET(request: NextRequest) {
  try {
    ...
    const [rentalRows, allContracts, itemRows, paymentRows, billRows, ledgerRows, renewalRows, buyoutRows, returnRows, lossRows, eventRows, memberRows, [settings]] = await Promise.all([
      ...
      db.select().from(receivableBills).where(and(eq(receivableBills.userId, userId), ...(from ? [gte(receivableBills.dueDate, from)] : []), ...(to ? [lte(receivableBills.dueDate, to)] : []))).orderBy(asc(receivableBills.dueDate)),
      db.select().from(accountLedger).where(and(eq(accountLedger.userId, userId), ...(from ? [gte(accountLedger.entryDate, from)] : []), ...(to ? [lte(accountLedger.entryDate, to)] : []))).orderBy(asc(accountLedger.entryDate)),
    ...
    addSheet(workbook, '设备明细', [
      { header: '合同编号', key: 'contractNo', width: 20 }, { header: '设备名称', key: 'deviceName', width: 24 }, { header: '类型', key: 'deviceType', width: 12 }, { header: '设备编号', key: 'deviceCode', width: 18 }, { header: '数量', key: 'quantity', width: 10 }, { header: '已买断', key: 'boughtOutQuantity', width: 10 }, { header: '已退租', key: 'returnedQuantity', width: 10 }, { header: '已丢失', key: 'lostQuantity', width: 10 }, { header: '开始日期', key: 'startDate', width: 14 }, { header: '结束日期', key: 'endDate', width: 14 }, { header: '月租', key: 'monthlyRent', width: 14 }, { header: '租金合计', key: 'totalRent', width: 14 },
      { header: '完整配置', key: 'configuration', width: 60 }, { header: '系统/其他配置', key: 'deviceConfig', width: 28 }, { header: 'CPU', key: 'cpu', width: 26 }, { header: '主板', key: 'motherboard', width: 26 }, { header: '内存', key: 'memory', width: 20 }, { header: '硬盘', key: 'storage', width: 22 }, { header: '显卡', key: 'graphicsCard', width: 26 }, { header: '电源', key: 'powerSupply', width: 22 }, { header: '机箱', key: 'caseModel', width: 22 }, { header: '显示器/支架', key: 'monitorInfo', width: 26 }, { header: '屏幕尺寸', key: 'screenSize', width: 16 }, { header: '分辨率', key: 'screenResolution', width: 18 }, { header: '刷新率', key: 'refreshRate', width: 14 }, { header: '面板类型', key: 'panelType', width: 18 }, { header: '色域', key: 'colorGamut', width: 20 }, { header: '接口', key: 'ports', width: 28 }, { header: '电池', key: 'batteryInfo', width: 24 }, { header: '适配器', key: 'adapterInfo', width: 22 }, { header: '配件', key: 'accessories', width: 30 },
    ], itemRows.map((row) => ({ ...row, contractNo: contractMap.get(row.rentalId) || row.rentalId, configuration: formatDeviceConfig(row, false) })))
