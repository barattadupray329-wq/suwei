...
export function parseBusinessExportQuery(searchParams: URLSearchParams) {
  const parsed = querySchema.safeParse(Object.fromEntries(searchParams))
  if (!parsed.success) throw Object.assign(new Error('日期格式无效'), { status: 400 })
  const { from, to } = parsed.data
  if (from && to && from > to) throw Object.assign(new Error('开始日期不能晚于结束日期'), { status: 400 })
  return { from, to }
}

export async function loadBusinessExportData(userId: string, from?: string, to?: string) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { counts, data } = await loadBusinessExportData(userId, from, to)
