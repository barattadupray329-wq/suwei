...
export async function POST(request: Request) {
  try {
    ...
    const identities = await verifyCustomerOtp(String(body.phone ?? ''), String(body.code ?? ''))
    return NextResponse.json({ ok: true, identities: { workspace: identities.workspace, customer: identities.customer } })
  } catch (error) {
