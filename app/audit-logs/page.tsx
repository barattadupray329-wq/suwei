...
export default async function AuditLogsPage() {
  ...
  return <main className="mx-auto flex max-w-7xl flex-col gap-6 p-4 md:p-6">
    <header><p className="text-sm font-medium text-primary">业务管理</p><h1 className="mt-1 text-2xl font-bold text-balance">业务操作记录</h1><p className="mt-1 text-sm text-muted-foreground">查看合同、收款及账号等重要事项的办理人员和时间。</p></header>
    <section className="overflow-hidden rounded-xl border bg-card">
      ...
      {!logs.length&&<p className="p-12 text-center text-sm text-muted-foreground">暂无业务操作记录。完成合同、收款或账号等重要操作后，相关记录会显示在这里。</p>}
    </section>
  </main>
}
