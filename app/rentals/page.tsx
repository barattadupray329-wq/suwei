...
export default async function RentalsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const filters = {
    query: value('query'), status: value('status') || '全部', startDate: value('startDate'), endDate: value('endDate'), assignee: value('assignee'), orderType, lifecycleStatus: 'active' as const, activeOnly: value('activeOnly') === '1', hasReceivable: value('hasReceivable') === '1', sort, page: Math.max(1, Number(value('page')) || 1),
  }
