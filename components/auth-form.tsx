      ...
      <div className="flex w-full flex-col overflow-y-auto p-6 sm:p-8 md:w-2/5 md:p-10 lg:p-12">
        ...
        {!showApplication ? <>
          <div className="mt-6 flex rounded-xl bg-muted p-1" role="tablist" aria-label="登录方式">
            ...
            <button type="button" role="tab" aria-selected={method === 'account'} onClick={() => { setMethod('account'); setError('') }} className={`flex h-10 flex-1 items-center justify-center gap-2 rounded-lg text-sm font-medium ${method === 'account' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}><KeyRound className="size-4" />业务工作台</button>
          </div>
        ...
        <div className="mt-6 flex items-start gap-2 rounded-lg bg-muted p-3 text-sm text-muted-foreground"><LockKeyhole className="mt-0.5 size-4 shrink-0 text-primary" /><div><p className="leading-6">客户经理账号由业务主管创建；邮箱只可作为账号文本，不用于邮箱验证。</p><button type="button" onClick={() => { setShowApplication((value) => !value); setError(''); setApplicationMessage('') }} className="mt-2 font-medium text-primary hover:underline">{showApplication ? '返回登录' : '申请管理员账号'}</button></div></div>
      </div>
    </section>
  </main>
}
