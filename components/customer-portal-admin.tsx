...
import { ArrowLeft, Ban, CheckCircle2, Clock3, Copy, ExternalLink, Eye, LogOut, QrCode, Search, ShieldCheck, Users } from 'lucide-react'
...
export function CustomerPortalAdmin({ customers }: { customers: any[] }) {
  ...
  const filtered = useMemo(() => customers.filter((customer) => {
    ...
    const current = customer.portal?.status === 'disabled' ? 'disabled' : 'active'
  ...
  const stats = {
    ...
    disabled: customers.filter((customer) => customer.portal?.status === 'disabled').length,
  }
  const copyLink = async () => { await navigator.clipboard.writeText(queryUrl); toast.success('客户查询链接已复制') }
  const changeStatus = (customer: any, next: 'active'|'disabled') => start(async () => { try { await setCustomerPortalStatus(customer.phone, next); toast.success(next === 'disabled' ? '已暂停该客户查询，现有会话已注销' : '已恢复客户查询') } catch (error) { toast.error(userErrorMessage(error)) } })
  ...
  return <main className="min-h-svh bg-background p-4 md:p-6"><div className="mx-auto flex max-w-[1400px] flex-col gap-6">
    ...
    <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">{[
      [Users, '可查询客户', stats.total], [CheckCircle2, '有效在租合同', stats.active], [Clock3, '已有登录记录', stats.recent], [Ban, '已暂停查询', stats.disabled],
    ...
    <section className="rounded-2xl border bg-card p-4"><div className="flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search className="absolute left-3 top-3 size-4 text-muted-foreground"/><span className="sr-only">搜索客户</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索姓名、公司或手机号" className="h-10 w-full rounded-xl border bg-background pl-9 pr-3 text-sm outline-none focus:border-primary"/></label><div className="flex gap-2">{(['all','active','disabled'] as const).map((item) => <button type="button" key={item} onClick={() => setStatus(item)} className={`h-10 rounded-xl px-4 text-sm font-medium ${status === item ? 'bg-primary text-primary-foreground' : 'border bg-background'}`}>{item === 'all' ? '全部' : item === 'active' ? '可查询' : '已暂停'}</button>)}</div></div></section>
    <section className="grid gap-3">{filtered.map((customer) => { const disabled = customer.portal?.status === 'disabled'; return <article key={customer.phone} className="flex flex-col justify-between gap-4 rounded-2xl border bg-card p-4 lg:flex-row lg:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold">{customer.customerName}{customer.customerCompany ? ` · ${customer.customerCompany}` : ''}</h2><span className={`rounded-full px-2 py-1 text-xs font-medium ${disabled ? 'bg-muted text-muted-foreground' : 'bg-primary/10 text-primary'}`}>{disabled ? '已暂停' : '可查询'}</span>{customer.hasSession && <span className="rounded-full bg-accent px-2 py-1 text-xs font-medium text-accent-foreground">当前有会话</span>}</div><p className="mt-2 text-sm text-muted-foreground">{customer.phone.slice(0,3)}****{customer.phone.slice(-4)} · {customer.contractCount} 份合同 · {customer.activeCount} 份有效在租</p><p className="mt-1 text-xs text-muted-foreground">{customer.portal?.lastLoginAt ? `最近验证：${new Date(customer.portal.lastLoginAt).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai',hour12:false})}` : '尚未通过短信验证登录'}</p></div><div className="flex flex-wrap gap-2"><Link href={`/customer-portals/preview/${encodeURIComponent(customer.phone)}`} className="inline-flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-sm font-medium text-primary-foreground"><Eye className="size-4"/>客户视角预览</Link>{customer.hasSession && <button disabled={pending} onClick={() => revoke(customer)} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-medium"><LogOut className="size-4"/>注销会话</button>}<button disabled={pending} onClick={() => changeStatus(customer, disabled ? 'active' : 'disabled')} className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium ${disabled ? 'bg-primary text-primary-foreground' : 'border border-destructive text-destructive'}`}>{disabled ? <CheckCircle2 className="size-4"/> : <Ban className="size-4"/>}{disabled ? '恢复查询' : '暂停查询'}</button></div></article>})}{!filtered.length && <div className="rounded-2xl border border-dashed bg-card p-10 text-center"><ShieldCheck className="mx-auto size-8 text-muted-foreground"/><p className="mt-3 font-medium">没有符合条件的客户</p><p className="mt-1 text-sm text-muted-foreground">客户有有效租赁合同后会自动出现在这里。</p></div>}</section>
