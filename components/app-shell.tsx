...
export function AppShell({ children, storeName, userName, role, permissions, version = 'dev' }: ShellProps) {
  ...
  const navigation = (mobile = false) => <nav aria-label={mobile ? '手机功能菜单' : '后台主导航'} className="flex flex-col gap-5">
    {visibleGroups.map((group) => <section key={group.label}><p className="mb-2 px-1 text-xs font-semibold text-muted-foreground">{group.label}</p>{mobile
      ? <div className="grid grid-cols-4 gap-1.5">{group.items.map(({ href, label, icon: Icon }) => <Link key={href} href={href} onClick={() => setMobileMenu(false)} className={`flex flex-col items-center gap-1 rounded-lg p-1.5 text-center transition-colors ${isActive(href) ? 'text-primary' : 'text-foreground hover:bg-muted'}`} aria-current={isActive(href) ? 'page' : undefined}><span className={`flex size-11 items-center justify-center rounded-xl ${isActive(href) ? 'bg-primary text-primary-foreground shadow-sm' : 'bg-muted text-foreground'}`}><Icon className="size-[18px]"/></span><span className="line-clamp-2 text-[11px] font-medium leading-tight">{label}</span></Link>)}</div>
