...
import { AlertTriangle, ArrowLeft, CalendarRange, Cloud, Download, FileSpreadsheet, HardDriveDownload, RefreshCw, ShieldCheck, Upload } from 'lucide-react'
...
export function BackupCenter({ storeName, version, counts, lastUpdated }: BackupCenterProps) {
  ...
  function exportDateParams() {
    if (from && to && from > to) { toast.error('开始日期不能晚于结束日期'); return null }
  ...
  async function downloadExport() {
    ...
    try {
      ...
      const anchor = document.createElement('a')
      anchor.href = url
      anchor.download = `${storeName.replace(/[\\/:*?"<>|]/g, '-')}租赁数据-${new Date().toISOString().slice(0, 10)}.json`
      document.body.appendChild(anchor)
      anchor.click()
      anchor.remove()
      URL.revokeObjectURL(url)
  ...
  async function downloadExcelExport() {
    ...
    setBusyAction('working')
    try {
      const response = await fetch(`/api/exports/business/excel?${params.toString()}`)
      if (!response.ok) {
        const body = await response.json().catch(() => null) as ApiError | null
        throw new Error(body?.error || '导出失败')
      }
      ...
      anchor.remove()
      URL.revokeObjectURL(url)
      toast.success('业务数据归档（Excel）已下载')
    } catch (error) {
      toast.error(userErrorMessage(error, '导出失败，请稍后重试'))
    } finally {
      setBusyAction(null)
