...
export function BackupCenter({ storeName, version, counts, lastUpdated }: BackupCenterProps) {
  ...
  const loadSnapshots = useCallback(() => fetch('/api/backups').then(response => response.json() as Promise<{ snapshots?: typeof snapshots }>).then(data => setSnapshots(data.snapshots || [])).catch(() => undefined), [])
