...
const displayStatus = (row: Row) => {
  ...
  if (row.endDate < today && !terminalStatuses.has(row.status)) return '逾期'
  return row.status
}
...
export function RentalRecords({ rows, total, overdueReceivable, page, pageCount, filters, assignees }: { rows: Row[]; total: number; overdueReceivable: string; page: number; pageCount: number; filters: Filters; assignees: Assignee[] }) {
  ...
  return <div className="page-container">
    ...
    <nav aria-label="租赁状态快捷筛选" className="flex flex-wrap gap-2">{['在租','逾期','已买断','已退租'].map((item) => <Link key={item} aria-current={filters.status === item ? 'page' : undefined} href={`/rentals?status=${encodeURIComponent(item)}`} className={`rounded-full px-4 py-2 text-sm font-semibold transition-opacity hover:opacity-80 ${statusTone(item)} ${filters.status === item ? 'ring-2 ring-foreground/20 ring-offset-2' : ''}`}>{item}</Link>)}<Link aria-current={filters.receivable === 'outstanding' ? 'page' : undefined} href="/rentals?receivable=outstanding&sort=outstanding" className={`rounded-full border px-4 py-2 text-sm font-semibold transition-colors ${filters.receivable === 'outstanding' ? 'border-destructive/30 bg-destructive/10 text-destructive ring-2 ring-destructive/20 ring-offset-2' : 'border-border bg-card text-foreground hover:border-primary'}`}>待收款</Link></nav>
