...
import { RentalWorkspaceTabs, rememberRentalWorkspace, saveRentalListReturn } from '@/components/rental-workspace-tabs'
...
export function RentalRecords({ rows, total, initialRentOutstanding, expectedReceivable, overdueReceivable, page, pageCount, filters, assignees }: { rows: Row[]; total: number; initialRentOutstanding: string; expectedReceivable: string; overdueReceivable: string; page: number; pageCount: number; filters: Filters; assignees: Assignee[] }) {
  ...
  const openDetail = (id: number) => {
    ...
    const href = `/rentals?${params}`
    if (row) rememberRentalWorkspace({ id: row.id, contractNo: row.contractNo, customerName: row.customerCompany || row.customerName, href })
    saveRentalListReturn(listParams.size ? `/rentals?${listParams}` : '/rentals')
    navigate(href)
  }
  ...
  return <div className="page-container" aria-busy={isPending}>
    ...
    <RentalWorkspaceTabs activeRental={null} listHref={pageHref(page)} />
