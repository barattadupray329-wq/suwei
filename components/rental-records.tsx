...
const terminalStatuses = new Set(['买断', '已买断', '已退租', '已结束', '已关闭', '已完成', '丢失'])
...
const displayStatus = (row: Row) => {
  ...
  if (row.endDate < today && !terminalStatuses.has(row.status)) {
    return Number(row.paidAmount) < Number(row.totalRent) ? '逾期' : '已到期'
  }
  return row.status
}
...
export function RentalRecords({ rows, total, page, pageCount, filters, assignees }: { rows: Row[]; total: number; page: number; pageCount: number; filters: Filters; assignees: Assignee[] }) {
  ...
  const openDetail = (id: number) => router.push(`/rentals?rental=${id}`)
