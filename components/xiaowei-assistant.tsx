...
export function XiaoweiAssistant() {
  ...
  const submit = (value = question, clarification?: string) => {
    ...
    const conversationHistory = [...messages, ...(answer && currentQuestion ? [{ question: currentQuestion, answer }] : [])].slice(-3).flatMap((message) => [
      { role: 'user' as const, content: message.question },
      { role: 'assistant' as const, content: `${message.answer.title}：${message.answer.summary}` },
    ])
    if (!clarification) {
    ...
    startTransition(async () => {
      try { setAnswer(await askXiaowei(clarification ? currentQuestion : clean, clarification, conversationContext, conversationHistory)) }
    ...
    {mounted && open && createPortal(<div className="fixed inset-0 z-[70] flex justify-end pointer-events-none sm:top-16">
      ...
      <aside role="dialog" aria-modal="true" aria-labelledby="xiaowei-title" className="pointer-events-auto relative flex h-full w-full flex-col overflow-hidden border-border bg-background shadow-2xl sm:w-[440px] sm:border-l sm:shadow-[-12px_0_32px_rgba(15,23,42,0.12)]">
        <header className="flex shrink-0 items-center justify-between border-b bg-primary px-5 py-4 text-primary-foreground">
          <div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-2xl bg-primary-foreground/15"><Bot className="size-6"/></span><div><h2 id="xiaowei-title" className="font-bold">小维</h2><p className="text-xs text-primary-foreground/75">智能经营助理 · 能聊天，也能办事</p></div></div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          {!answer ? <div className="flex flex-col gap-5">
            <section className="rounded-2xl border bg-card p-5"><div className="flex items-start gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Sparkles className="size-5"/></span><div><h3 className="font-bold">你好，我是小维</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">你可以直接和我聊天，也可以让我查询合同、设备、账单和逾期情况。短信提醒等操作会先给你确认，再安全执行。</p></div></div></section>
